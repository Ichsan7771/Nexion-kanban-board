...
export const Item = ({
    ...
    const onCreate = (event: React.MouseEvent<HTMLDivElement, MouseEvent>) => {
        event.stopPropagation();
        const promise = create({ title: "Tak Berjudul", parentDocument: id })
        .then((documentId) => {
            if (!expanded) {
                onExpand?.();
            }
        ...
        >
            {!!id && (
                ...
                >
                    ...
                    <ChevronIcon
                    className="h-4 w-4 shrink-0 mr-2 text-muted-foreground/50"
                    />
                </div>
            )}
                ...
                <div className="ml-auto flex items-center gap-x-2">
                    <DropdownMenu>
                        ...
                        >
                            <div
                            role="button"
                            className="opacityy-0 group-hover:opacity-100 h-full ml-auto 
                    ...
                    rounded-sm hover:bg-neutral-300 dark:hover:bg-neutral-600"
                    >
                        <Plus className="h- w-4 text-muted-foreground"/>
                    </div>
                </div>
            )}
        </div>
